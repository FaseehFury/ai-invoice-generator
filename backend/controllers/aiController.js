...
const parseInvoiceFromText = async (req, res) => {
  ...
  try {
    const prompt = `You are an expert invoice data extraction AI. Analyze the following text and extract the relative information to create an invoice.
    THe output MUST must be a valid JSON object.
    ...
    Extract rhe data and provide only JSON object
`;
...
const generateReminderEmail = async (req, res) => {
  ...
  try {
    ...
    const prompt = `
you are a professional and ploite accounting assistant. Write a friendly reminder email to a client about an overdue or upcoming incoive payment.
...
const getDashboardSummary = async (req, res) => {
  try {
    ...
    -Total revenue form paid invoices: ${totalRevenue.toFixed(2)}
    -Total outstanding amount form unpaid/pending invoices: ${totalOutstanding.toFixed(
      2
    )}
    ...
    For example, if there is a high outstanding amount, suggest pending reminders. If revenue is high, Be encouraging.
