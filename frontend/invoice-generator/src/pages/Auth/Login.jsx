...
const Login = () => {
  ...
  const [toucehed, setTouched] = useState({
    email: false,
    password: false,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const respone = await axiosInstance.post(API_PATHS.AUTH.LOGIN, formData);
      if (respone.status === 200) {
        const { token } = respone.data;

        if (token) {
          setSuccess("Login successfull");
          login(respone.data, token);
        ...
        setError(respone.data.message || "Invalid crendentials");
      }
    } catch (err) {
      if (err.respone && err.respone.data && err.respone.data.message) {
    ...
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full  pl-12 pr-4 py-3 border rounded-lg focus:ring-2 focus:border-transparent outline-none transition-all ${
                  fieldErrors.email && toucehed.email
          ...
          {/* password */}
          <div>
            <label className="block text-sm font-medium text-hray-700 mb-2">
              Password
            </label>
            <div className="relative">
              <Lock className="absolute left-4 top-1/2 transform -translate-y-1 text-gray-400 w-5 h-5" />
              <input
                name="password"
                ...
                className={`w-full pl-12 pr-12 py-3 border rounded-lg focus:ring-2 focus:border-transparent outline-none transition-all ${
                  fieldErrors.password && toucehed.password
                    ? "border-red-300 focus:ring-red-500"
                    : "border-hray-300 focus:ring-bllack"
                }`}
