    ...
    <button
      ...
      className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 group ${
        isActive
          ...
          : "text-gray-600 hover:bg0gray-50 hover:text-gray-900"
      }`}
    >
      <Icon
...
const DashboardLayout = ({ children, activeMenu }) => {
  ...
  //close dropdown when clickig outside
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50  -transform duration-300 trasnform ${
          isMobile
            ? sidebarOpen
              ? "translate-x-0"
              : "-translate-x-full"
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <button
            className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg text-gray-600 not-first:hover:bg-gray-50 hover:text-gray-900 transition-all duration-200"
            onClick={logout}
          >
      ...
      <div
        className={`flex-1 flex flex-col transition-all duration-300 ${
          isMobile ? "ml-0" : sidebarCollapsed ? "ml-10" : "ml-64"
        }`}
      >
        {/* Top navbar */}
